...
function patchAppJson(file) {
  ...
  if (app.window) {
    app.window.navigationBarTitleText = '租管家Pro'
  }
...
function patchProjectConfig(file) {
  if (!fs.existsSync(file)) return
  const config = JSON.parse(read(file))
  config.description = 'IRent 小程序项目配置文件'
  if (!config.projectname) config.projectname = 'IRent'
...
function main() {
  ...
  patchProjectConfig(path.join(root, 'project.config.json'))
