...
> 2026-08-02 更新：当前生产服务已部署在腾讯云轻量应用服务器，入口为
> `https://irent.antoniolq.top`，使用 Caddy 反向代理至 Docker 中的 Express API 与 PostgreSQL。
> 小程序 request、uploadFile、downloadFile 合法域名均应配置为该 HTTPS 域名。

## 首次本机数据迁移

首次启用共享数据时，请先在**保存现有业务数据的手机**上更新小程序并进入“工作台”。
若云端还没有房源，应用会将本机的房源、入住/退租归档、租金期次、账单、收费、抄表和历史操作一次性写入云端；云端已有数据时接口会拒绝覆盖。

- 完成首台初始化后，再让第二台手机进入工作台，即会读取同一份云端数据。
- 迁移不会上传仅存在于手机沙盒内的旧图片文件；后续在应用内上传的证件、合同和凭证会按云端附件流程共享。
