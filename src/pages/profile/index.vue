  ...
  <view class="h-screen bg-slate-50 text-slate-800">
    <view class="mx-auto max-w-md h-screen flex flex-col shadow-2xl bg-slate-50 relative overflow-hidden">
      <view class="bg-white-80 border-b px-5 pb-3 border-slate-200-60 relative shrink-0 sticky-header z-20 shadow-soft" :style="{ paddingTop: headerTopPadding + 'px' }">
        <view class="flex items-center justify-between gap-3">
          <view class="flex items-center gap-3 min-w-0">
            <button
              v-if="subPage"
              class="nav-icon-button tap-scale"
              @click="subPage = ''"
            >
        ...
        <view v-if="!subPage" class="mt-4 p-6 rounded-2xl profile-hero text-white relative overflow-hidden border-none shadow-lg">
          ...
          <view class="flex items-center gap-4 relative z-10">
            <view class="w-16 h-16 bg-white-20 rounded-full flex items-center justify-center text-white font-black text-2xl border border-white-20">
              {{ profileInitial }}
            </view>
            <view class="min-w-0">
              <view class="text-xl font-black truncate">{{ profileName }}</view>
              <view class="flex gap-2 mt-2 flex-wrap">
                <view class="profile-hero-chip">{{ profileUi.plan }}</view>
                <view class="profile-hero-chip">{{ profileUi.verified }}</view>
                <view class="profile-hero-chip">{{ profileUi.storage }}</view>
              </view>
            </view>
          </view>
          ...
          <view class="grid grid-cols-3 gap-4 mt-6 pt-4 border-t border-white-20 relative z-10">
            <view class="text-center">
              ...
              <view class="text-3xs text-slate-200 font-medium mt-1">{{ profileUi.properties }}</view>
            </view>
            <view class="text-center border-l border-white-20">
              <view class="text-2xl font-black font-mono">{{ stats.totalRooms }}</view>
              <view class="text-3xs text-slate-200 font-medium mt-1">{{ profileUi.rooms }}</view>
            </view>
            <view class="text-center border-l border-white-20">
              <view class="text-2xl font-black font-mono text-blue-200">{{ stats.rentedRooms }}</view>
              <view class="text-3xs text-slate-200 font-medium mt-1">{{ profileUi.rented }}</view>
            </view>
          </view>
        </view>      </view>
        ...
        <view class="p-5 stack-5" style="padding-bottom: 32rpx;">
          <view v-if="!subPage" class="stack-5">
            ...
            <view class="overflow-hidden surface-card" :class="UI.card">
              <view
                v-for="(item, i) in menuA"
                :key="item.id"
                class="p-4 flex items-center justify-between tap-scale"
                :class="i !== menuA.length - 1 ? 'border-b border-slate-100' : ''"
                @click="openSubPage(item.id)"
              >
                <view class="flex items-center gap-3 min-w-0">
                  <view class="w-8 h-8 rounded-lg flex items-center justify-center font-black text-sm" :class="item.bg">
                    <text :class="item.color">{{ item.icon }}</text>
                  </view>
                  <view class="font-bold text-slate-700 text-sm truncate">{{ item.label }}</view>
                </view>
            ...
            <view class="overflow-hidden surface-card" :class="UI.card">
              <view
                v-for="(item, i) in menuB"
                :key="item.id"
                class="p-4 flex items-center justify-between tap-scale"
                :class="i !== menuB.length - 1 ? 'border-b border-slate-100' : ''"
                @click="openSubPage(item.id)"
              >
                <view class="flex items-center gap-3 min-w-0">
                  <view class="w-8 h-8 rounded-lg flex items-center justify-center font-black text-sm" :class="item.bg">
            ...
            <view class="pt-2 pb-6">
              <button class="w-full py-3 rounded-xl btn-slate font-bold tap-scale mb-3" @click="resetDemo">
                重置演示数据
              </button>
              <button class="w-full py-3 rounded-xl btn-soft text-rose-600 font-bold tap-scale" @click="logout">
                安全退出登录
              </button>
            </view>
          </view>
          ...
          <view v-else-if="subPage === 'allDocuments'" class="stack-4">
            <view class="relative">
              <view
                class="absolute left-4 w-7 h-7 rounded-xl bg-slate-100 text-slate-500 flex items-center justify-center font-black text-xs"
              ...
              <input
                v-model="docSearch"
                type="text"
                class="w-full pl-12 pr-4 py-3 rounded-2xl input-soft font-medium text-slate-800"
                placeholder="搜索姓名 / 房号 / 手机号"
              />
            </view>

            <view v-if="filteredDocs.length === 0" class="text-center py-12 bg-slate-50 rounded-2xl border border-slate-200 border-dashed text-slate-400 font-medium text-sm">
              暂无匹配的档案记录
            </view>
            ...
            <view v-else class="stack-3">
              <view v-for="doc in filteredDocs" :key="doc.id" class="p-4 rounded-2xl surface-card">
                ...
                <view class="flex gap-2 mb-3">
                  <view class="px-2 py-1 rounded-lg text-3xs font-black border" :class="doc.hasIdCardPic ? 'bg-blue-50 text-blue-600 border-blue-200' : 'bg-slate-50 text-slate-400 border-slate-200'">
                    {{ doc.hasIdCardPic ? '身份证已归档' : '身份证缺失' }}
                  </view>
                  <view class="px-2 py-1 rounded-lg text-3xs font-black border" :class="doc.hasContract ? 'bg-indigo-50 text-indigo-600 border-indigo-200' : 'bg-slate-50 text-slate-400 border-slate-200'">
                    {{ doc.hasContract ? '合同已归档' : '合同缺失' }}
                  </view>
                </view>
                <view class="flex gap-2">
                  <button
                    class="flex-1 py-2 rounded-xl text-xs font-black tap-scale"
                    :class="doc.hasIdCardPic ? 'bg-blue-50 text-blue-600 border border-blue-200' : 'bg-slate-50 text-slate-400 border border-slate-200'"
                    :disabled="!doc.hasIdCardPic"
                    @click="openAttachment('idCard', doc)"
                  >
                    {{ doc.hasIdCardPic ? '查阅身份证' : '无证件' }}
                  </button>
                  <button
                    class="flex-1 py-2 rounded-xl text-xs font-black tap-scale"
                    :class="doc.hasContract ? 'bg-indigo-50 text-indigo-600 border border-indigo-200' : 'bg-slate-50 text-slate-400 border border-slate-200'"
                    :disabled="!doc.hasContract"
                    @click="openAttachment('contract', doc)"
                  >
                    {{ doc.hasContract ? '查电子合同' : '无合同' }}
                  </button>
                </view>
              </view>
            </view>
          ...
          <view v-else-if="subPage === 'utilityTemplate'" class="stack-4">
            <view class="p-5 rounded-2xl bg-orange-50 border border-orange-200">
              <view class="flex items-start gap-3">
                ...
                <view class="text-sm text-orange-800 font-medium leading-relaxed">
                设置全局默认水电单价。新建房源时默认采用这套模板，房间层级仍可单独覆盖。
                </view>
              </view>
            </view>
            ...
            <view class="p-5 rounded-2xl surface-card">
              ...
              <view class="grid grid-cols-2 gap-3 mt-4">
                ...
                <view class="p-4 rounded-2xl surface-muted">
                  <view class="text-xs text-slate-500 font-bold">电(元/度)</view>
          ...
          <view v-else-if="subPage === 'exportReport'" class="stack-4">
            <view class="p-5 rounded-2xl bg-emerald-50 border border-emerald-200">
              <view class="flex items-center gap-3">
                ...
                <view class="text-sm text-emerald-800 font-medium leading-relaxed">
                支持将账单与租客明细导出为 Excel 文件，用于本地存档。此处先做 UI 占位，下一步接真实导出逻辑。
                </view>
              </view>
            </view>
            ...
            <view class="p-5 rounded-2xl surface-card stack-4">
              <view>
                ...
                <view class="text-xs text-slate-400 font-medium mt-1">当前 demo 仅展示 UI。</view>
              </view>
              <view class="p-4 rounded-2xl surface-muted">
          ...
          <view v-else-if="subPage === 'contractLibrary'" class="stack-4">
            ...
            <view class="overflow-hidden surface-card" :class="UI.card">
              ...
              <view>
                <view
                  v-for="(tpl, i) in contractTemplates"
                  :key="tpl.id"
                  class="p-4 flex items-center justify-between gap-3"
                  :class="i !== contractTemplates.length - 1 ? 'border-b border-slate-100' : ''"
                >
                  <view class="flex items-center gap-3 min-w-0">
                  ...
                  <button class="px-3 py-2 rounded-xl bg-blue-50 text-blue-600 text-xs font-black border border-blue-200 tap-scale" @click="openTemplatePreview(tpl)">
                    预览
                  </button>
                </view>
              </view>
          ...
          <view v-else-if="subPage === 'autoReminder'" class="stack-4">
            <view class="p-5 rounded-2xl bg-amber-50 border border-amber-200">
              <view class="flex items-start gap-3">
                <view class="w-9 h-9 rounded-xl bg-white text-amber-500 flex items-center justify-center font-black shadow-soft shrink-0">钟</view>
                <view class="text-sm text-amber-800 font-medium leading-relaxed">
                这里是“到期提醒机制”的 UI 规划页。当前仅做自我管理开关，不会生成催缴文案，也不会推送微信消息。
                </view>
              </view>
            </view>
            ...
            <view class="p-5 rounded-2xl surface-card stack-5">
              <view class="flex justify-between items-center gap-3">
                <view class="min-w-0">
                  ...
                  <view class="text-xs text-slate-400 font-medium mt-1">用于提醒自己准备收款。</view>
                </view>
                <view class="w-12 h-7 rounded-full p-1 transition-colors shrink-0" :class="reminderForm.advance ? 'bg-amber-500' : 'bg-slate-200'" @click="reminderForm.advance = !reminderForm.advance">
                  <view class="w-5 h-5 bg-white rounded-full transition-transform shadow-soft" :class="reminderForm.advance ? 'translate-x-5' : 'translate-x-0'"></view>
                </view>
              </view>
              ...
              <view class="flex justify-between items-center gap-3">
                <view class="min-w-0">
                  ...
                  <view class="text-xs text-slate-400 font-medium mt-1">账单到期当天提示一下。</view>
                </view>
                <view class="w-12 h-7 rounded-full p-1 transition-colors shrink-0" :class="reminderForm.overdue ? 'bg-rose-500' : 'bg-slate-200'" @click="reminderForm.overdue = !reminderForm.overdue">
                  <view class="w-5 h-5 bg-white rounded-full transition-transform shadow-soft" :class="reminderForm.overdue ? 'translate-x-5' : 'translate-x-0'"></view>
                </view>
              </view>
              ...
              <view class="flex justify-between items-center gap-3">
                <view class="min-w-0">
                  <view class="font-black text-slate-800 text-base">逾期 3 天警告</view>
                  <view class="text-xs text-slate-400 font-medium mt-1">用于标记重点房间，方便跟进。</view>
                </view>
                <view class="w-12 h-7 rounded-full p-1 transition-colors shrink-0" :class="reminderForm.warnThreeDays ? 'bg-rose-600' : 'bg-slate-200'" @click="reminderForm.warnThreeDays = !reminderForm.warnThreeDays">
                  <view class="w-5 h-5 bg-white rounded-full transition-transform shadow-soft" :class="reminderForm.warnThreeDays ? 'translate-x-5' : 'translate-x-0'"></view>
                </view>
              </view>
            </view>
          ...
          <view v-else class="stack-4">
            <view class="p-5 rounded-2xl surface-card">
              ...
              <view class="text-sm text-slate-600 font-medium leading-relaxed mt-2">
                当前先把核心闭环（房态 + 自我记账 + 历史可追溯）做稳，再逐项补全。
              </view>
            </view>
          </view>
        </view>
      ...
      <view v-if="subPage === 'utilityTemplate'" class="absolute inset-x-0 bottom-0 p-5 bg-white border-t border-slate-200-60 shadow-top-soft">
        <button class="w-full py-4 rounded-xl btn-blue font-bold tap-scale" @click="saveConfig">
          保存并应用全局设置
        </button>
      </view>
      ...
      <view v-if="subPage === 'autoReminder'" class="absolute inset-x-0 bottom-0 p-5 bg-white border-t border-slate-200-60 shadow-top-soft">
        <button class="w-full py-4 rounded-xl btn-blue font-bold tap-scale" @click="saveReminder">
          保存并应用提醒设置
        </button>
      </view>
      ...
      <view v-if="subPage === 'exportReport'" class="absolute inset-x-0 bottom-0 p-5 bg-white border-t border-slate-200-60 shadow-top-soft">
        <button class="w-full py-4 rounded-xl btn-emerald font-bold tap-scale" @click="toastSoon">
          生成并导出 Excel
        </button>
      </view>

      <BottomDrawer :open="attachmentOpen" title="档案预览" subtitle="图片占位，后续接入 R2 存真实文件" @close="closeAttachment">
        <view v-if="attachmentInfo" class="stack-3">
          ...
          <view class="p-4 rounded-2xl surface-card">
            ...
            <view class="text-2xs text-slate-400 font-medium mt-3">
              demo 版本仅展示结构与入口，真实文件会保存到对象存储并做鉴权访问。
            </view>
          </view>
        </view>
      </BottomDrawer>
...
const subPage = ref('') // '' means root, otherwise sub page id

const profileName = ref('李房东')
const profileInitial = computed(() => String(profileName.value || '房').trim().slice(0, 1) || '房')
...
const contractTemplates = [
  ...
  { id: 'tpl_2', title: '商铺专属合同_通用版', lastUsed: '1个月前' },
]
...
const menuA = [
  { id: 'allDocuments', icon: '盾', label: '租客证件与合同归档', bg: 'bg-blue-50', color: 'text-blue-600' },
  { id: 'utilityTemplate', icon: '水', label: '默认水电单价模板', bg: 'bg-orange-50', color: 'text-orange-600' },
  { id: 'contractLibrary', icon: '合', label: '电子租赁合同库', bg: 'bg-indigo-50', color: 'text-indigo-600' },
]

const menuB = [
  { id: 'autoReminder', icon: '钟', label: '到期提醒机制(规划)', bg: 'bg-amber-50', color: 'text-amber-600' },
  { id: 'exportReport', icon: '表', label: '数据报表导出 (Excel)', bg: 'bg-emerald-50', color: 'text-emerald-600' },
]
...
const pageSubtitle = computed(() => {
  ...
  if (subPage.value === 'allDocuments') return '按姓名/房号/手机号检索'
  if (subPage.value === 'contractLibrary') return '上传/预览模板(模拟)'
  if (subPage.value === 'autoReminder') return '仅自我管理，不推送文案'
  return '下一步完善'
})
...
function saveConfig() {
  ...
  try {
    uni.setStorageSync('global_config_v1', globalConfig.value)
  } catch {
    // ignore
  }
...
function saveReminder() {
  try {
    uni.setStorageSync('reminder_config_v1', reminderForm.value)
  } catch {
    // ignore
  }
  uni.showToast({ title: '已保存', icon: 'success' })
...
function showAbout() {
  uni.showModal({
    title: '关于',
    content: '这是原型高保真复刻的 v1（demo 数据）。当前重点是房态与自我记账闭环，后续会接入后端与 Cloudflare R2。',
    showCancel: false,
  })
}
