<template>
  <BaseCenteredModal :open="open" :title="title" :subtitle="subtitle" body-class="stack-3" @close="emit('close')">
    ...
    <view class="charge-drawer-section">
      ...
      <view class="charge-drawer-entry-row"><view class="charge-drawer-input-wrap"><view class="charge-drawer-currency">￥</view><input :value="modelValue" type="number" class="charge-drawer-input" :placeholder="placeholder" @input="emit('update:modelValue', $event.detail.value)" /></view><button class="charge-drawer-upload-button tap-scale" :class="receiptPicked ? 'charge-drawer-upload-button-done' : ''" @click="emit('pick-receipt')">{{ receiptPicked ? '已上传' : '上传凭证' }}</button></view>
      <view class="charge-drawer-inline-status" :class="receiptPicked ? 'charge-drawer-inline-status-done' : ''">{{ receiptPicked ? receiptFileName : receiptPendingText }}</view>
      <view v-if="helperText" class="charge-drawer-helper">{{ helperText }}</view>
    </view>
    <template #footer><ActionFooterRow secondary-label="取消" :primary-label="confirmLabel" :primary-disabled="confirmDisabled" @secondary="emit('close')" @primary="emit('confirm')"><template #primarySuffix><text v-if="confirmBadge" class="charge-drawer-confirm-badge">{{ confirmBadge }}</text></template></ActionFooterRow></template>
  </BaseCenteredModal>
</template>
...
const props = defineProps({
  ...
  confirmLabel: { type: String, default: '确认收款' }, confirmDisabled: Boolean, confirmBadge: String, helperText: String, heroTone: { type: String, default: 'blue' },
})
