<template>
  <BaseCenteredModal :open="open" :title="title" :subtitle="subtitle" body-class="stack-3" @close="emit('close')">
    <view class="checkout-status-list">
      <view class="checkout-status-row">
        <view class="checkout-status-top"><view class="checkout-status-label">租金缴纳情况</view><view class="checkout-status-lamp" :class="rentStatusClass"></view></view>
        <view class="checkout-status-main">{{ rentStatusText }}</view><view class="checkout-status-sub">{{ rentStatusNote }}</view>
      </view>
      <view class="checkout-status-row">
        <view class="checkout-status-top"><view class="checkout-status-label">附加费用情况</view><view class="checkout-status-lamp" :class="utilityStatusClass"></view></view>
...
<style>
.checkout-status-list { overflow: hidden; border-radius: 22rpx; background: #f8fafc; }
.checkout-status-row { padding: 20rpx; border-bottom: 1rpx solid #e2e8f0; }.checkout-status-row:last-child { border-bottom: 0; }
.checkout-field-card { padding: 20rpx; border-radius: 22rpx; background: #f8fafc; }
.checkout-status-top { display: flex; align-items: center; justify-content: space-between; gap: 12rpx; }.checkout-status-label,.checkout-field-label { font-size: 24rpx; font-weight: 600; color: #475569; }.checkout-status-main { margin-top: 10rpx; font-size: 30rpx; font-weight: 700; color: #0f172a; }.checkout-status-sub,.checkout-field-hint { margin-top: 6rpx; font-size: 22rpx; color: #94a3b8; }
