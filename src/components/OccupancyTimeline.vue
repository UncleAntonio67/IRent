        ...
        <view class="flex-1 pb-1">
          <view class="p-3 rounded-2xl border shadow-soft" :class="cardClass(occ)">
            <view class="flex items-start justify-between gap-3">
              <view class="min-w-0">
                <view class="font-black text-slate-900 text-sm truncate">{{ occ.tenant || '未知租客' }}</view>
                <view class="text-2xs text-slate-500 font-mono mt-1">{{ formatRange(occ) }}</view>
              </view>
            ...
            <view class="grid grid-cols-3 gap-2 mt-3">
              <view class="p-2 rounded-xl bg-white-80 border border-slate-200-60">
                <view class="text-2xs text-slate-500 font-bold">手机号</view>
                <view class="text-xs font-black text-slate-900 mt-1 truncate">{{ occ.phone || '-' }}</view>
              </view>
              <view class="p-2 rounded-xl bg-white-80 border border-slate-200-60">
                <view class="text-2xs text-slate-500 font-bold">租金总额</view>
                <view class="text-xs font-black text-slate-900 font-mono mt-1">￥{{ occ.rentTotal || 0 }}</view>
              </view>
              <view class="p-2 rounded-xl bg-white-80 border border-slate-200-60">
                <view class="text-2xs text-slate-500 font-bold">附加费</view>
                <view class="text-xs font-black text-slate-900 font-mono mt-1">￥{{ occ.extraTotal || 0 }}</view>
              </view>
            </view>
          </view>
        </view>
