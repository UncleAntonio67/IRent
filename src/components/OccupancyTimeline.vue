<template>
  <view>
    <view v-if="items.length === 0" class="p-4 rounded-2xl bg-white shadow-soft">
    ...
    <view v-else class="stack-2">
      <view v-for="(occ, idx) in items" :key="occ.id" class="flex gap-3">
        <view class="w-6 flex flex-col items-center shrink-0">
          <view class="w-3 h-3 rounded-full shadow-soft" :class="dotClass(occ)"></view>
          <view v-if="idx !== items.length - 1" class="w-px flex-1 mt-2 bg-slate-200 opacity-70"></view>
        </view>

        <view class="flex-1 pb-1">
          <view class="p-3 rounded-2xl shadow-soft" :class="cardClass(occ)">
            <view class="flex items-start justify-between gap-3">
              <view class="min-w-0">
                <view class="text-sm font-semibold text-slate-900 truncate">{{ occ.tenant || '未填写租客' }}</view>
                <view class="text-2xs text-slate-500 font-medium mt-1">{{ formatRange(occ) }}</view>
              </view>
              <view class="text-2xs font-semibold px-3 py-1 rounded-full shrink-0" :class="tagClass(occ)">
                {{ occ.status === 'active' ? '当前' : '历史' }}
              </view>
            </view>

            <view class="grid grid-cols-3 gap-2 mt-3">
              <view class="p-2 rounded-xl bg-white-80">
