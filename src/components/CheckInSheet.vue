<template>
  <view v-if="open" class="fixed inset-0 z-50 bg-slate-900-50 flex items-end justify-center" @click="emit('close')">
    ...
    <DateSelectionModal v-if="dateDrawerOpen" :open="dateDrawerOpen" title="选择租期开始" subtitle="默认今天，可前后微调" @close="dateDrawerOpen=false"><view class="stack-2"><view class="p-3 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-between gap-2"><button class="date-step-button tap-scale" @click="shiftLeaseStart(-1)">前一天</button><view class="text-center min-w-0 flex-1"><view class="text-sm font-black text-slate-800">{{ form.leaseStart }}</view><view class="text-2xs text-slate-400 mt-1">默认今天，可前后调整。</view></view><button class="date-step-button tap-scale" @click="shiftLeaseStart(1)">后一天</button></view><view class="grid grid-cols-3 gap-2"><button class="date-chip tap-scale" @click="shiftLeaseStart(-30)">前 30 天</button><button class="date-chip tap-scale" @click="setLeaseStart(todayString)">今天</button><button class="date-chip tap-scale" @click="shiftLeaseStart(30)">后 30 天</button></view></view><template #footer><button class="w-full py-3 rounded-xl btn-blue font-bold tap-scale" @click="dateDrawerOpen=false">确认日期</button></template></DateSelectionModal>
...
const roomPhotoSrc = (file) => resolveOfflineImageSrc(file)
...
async function uploadRoomScopedFile(type, rawFile, { successTitle = '已上传图片', preview = false } = {}) {
  ...
  if (canUseCloudBackup() && type !== 'receipt') {
    ...
    if (preview) previewFile(prepared)
    return prepared
  }
...
async function pickAttachment(type) {
  ...
  try {
    attachments.value[type] = await uploadRoomScopedFile(type, await chooseSingleImage({ fallbackPrefix: type }))
  } catch (error) {
  ...
  try {
    ...
    if (canUseCloudBackup()) {
      ...
      enqueueSyncTask({
        ...
        payload: {
          type: 'roomPhoto',
          file: buildChosenFile('room_photo', chosen),
        },
      })
...
async function confirmCheckIn() {
  ...
  if (canUseCloudBackup()) {
    const attachmentIds = [...new Set([
      attachments.value.idCard?.id,
      attachments.value.contract?.id,
